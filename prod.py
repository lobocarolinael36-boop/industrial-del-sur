...
def alta_producto():
    ...
    if not precio.get().isdigit():
        messagebox.showerror("Error", "El Precio debe ser un numero entero."); return
...
def modificar_producto():
    if not codigo.get():
        messagebox.showwarning("Atencion", "Ingrese el Identificador."); return
    if not precio.get().isdigit():
        messagebox.showerror("Error", "El Precio debe ser numerico."); return
    productos = leer_productos(); found = False
    for p in productos:
        if p[0] == codigo.get():
...
campo(1, "Detalle:",       detalle)
campo(2, "Talle:", talle, ttk.Combobox(ff, textvariable=talle,
      values=["XS","S","M","L","XL","XXL"], state="readonly", font=("Arial",10)))
campo(3, "Color:", color, ttk.Combobox(ff, textvariable=color,
      values=["BLANCO","NEGRO","AZUL","ROJO","AMARILLO"], state="readonly", font=("Arial",10)))
...
for col, txt, w, anch in [
    ("id","ID",52,"center"), ("detalle","Detalle",148,"w"),
