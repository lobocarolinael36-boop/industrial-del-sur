...
# ── Validaciones ─────────────────────────────────────────────────────────
def validar_campos_alta():
    if not all([id_usr.get(), nombre.get(), apellido.get(), password.get()]):
        messagebox.showwarning("Atencion", "Complete todos los campos obligatorios."); return False
    if not id_usr.get().isdigit():
        messagebox.showerror("Error", "El ID (DNI) debe ser estrictamente numerico."); return False
    if len(id_usr.get()) < 7 or len(id_usr.get()) > 8:
        messagebox.showerror("Error", "El DNI debe tener entre 7 y 8 digitos."); return False
    if len(password.get()) < 4:
        messagebox.showerror("Error", "La contrasena debe tener al menos 4 caracteres."); return False
    return True

# ── Operaciones ABM ───────────────────────────────────────────────────────
def alta_usuario():
    if not validar_campos_alta(): return
    ...
    usuarios.append([id_usr.get(), nombre.get(), apellido.get(),
                     password.get(), rol.get(), estado.get(), fecha])
...
def baja_usuario():
    ...
    for u in usuarios:
        if u[0] == id_usr.get():
            u[5] = "INACTIVO"; u[6] = fecha_ahora(); found = True
...
def modificar_usuario():
    ...
    guardar_usuarios(usuarios); cargar_tabla()
    if found:
        messagebox.showinfo("Modificacion", "Usuario modificado correctamente.\nFecha de modificacion actualizada.")
    else:
        messagebox.showerror("Error", "Usuario no encontrado.")
...
def seleccionar(event):
    ...
    for u in leer_usuarios():
        if u[0] == dni_sel:
            ...
            password.set(u[3]); rol.set(u[4]); estado.set(u[5])
            fecha_mod.set(u[6])
            break
...
def campo_lbl(row, text):
    tk.Label(ff, text=text, bg=WHITE, fg=TEXT, font=("Arial",10,"bold"), anchor="w").grid(
...
tk.Label(ff, text="(auto) dejala vacia en contrasena para no cambiarla",
         bg=WHITE, fg=NEUTRAL, font=("Arial",8), anchor="w").grid(
...
tabla = ttk.Treeview(tf,
    columns=("id","nombre","apellido","password","rol","estado","fecha"),
...
for col, txt, w, anch in [
    ("id",       "DNI",          90, "center"),
    ("nombre",   "Nombre",       98, "w"),
    ("apellido", "Apellido",     98, "w"),
    ("password", "Contrasena",   90, "center"),
    ("rol",      "Rol",         108, "center"),
    ("estado",   "Estado",       78, "center"),
    ("fecha",    "Ultima Mod.",  148, "center")]:
